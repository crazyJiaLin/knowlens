...
{
  "registry-mirrors": [
    "https://8awh0477.mirror.aliyuncs.com"
  ]
}
EOF
...
4. **如果镜像加速器导致 "not found" 错误**：

   某些镜像加速器可能无法正确代理所有镜像，可以尝试以下方法：
   ...
   **方法一：临时禁用镜像加速器测试**：
   ...
   # 尝试直接拉取（可能很慢，但可以验证镜像是否存在）
   docker pull node:20-alpine
   ```

   **方法二：使用阿里云容器镜像服务的公共镜像仓库**：
   ```bash
   # 直接从阿里云拉取镜像
   ...
   **方法四：检查并修复 DNS 问题**：
   ...
   # 测试 DNS 解析
   nslookup docker.io
   nslookup 8awh0477.mirror.aliyuncs.com
   
   # 如果 DNS 解析失败，更换 DNS 服务器
   echo "nameserver 8.8.8.8" > /etc/resolv.conf
   echo "nameserver 114.114.114.114" >> /etc/resolv.conf
   ...
   **方法五：修改 Dockerfile 使用可用的镜像源**（推荐）：

   如果镜像加速器一直有问题，可以直接修改 Dockerfile 使用其他可用的镜像源：
   ...
   # 修改前端 Dockerfile
   sed -i 's|FROM node:20-alpine|FROM registry.cn-hangzhou.aliyuncs.com/acs/node:20-alpine|g' frontend/Dockerfile
   sed -i 's|FROM nginx:alpine|FROM registry.cn-hangzhou.aliyuncs.com/acs/nginx:alpine|g' frontend/Dockerfile
   
   # 修改后端 Dockerfile
   sed -i 's|FROM node:20-alpine|FROM registry.cn-hangzhou.aliyuncs.com/acs/node:20-alpine|g' backend/Dockerfile
   ```
   ...
   ```bash
   # 使用 DaoCloud 镜像源
   sed -i 's|FROM node:20-alpine|FROM docker.m.daocloud.io/library/node:20-alpine|g' frontend/Dockerfile
   sed -i 's|FROM nginx:alpine|FROM docker.m.daocloud.io/library/nginx:alpine|g' frontend/Dockerfile
   sed -i 's|FROM node:20-alpine|FROM docker.m.daocloud.io/library/node:20-alpine|g' backend/Dockerfile
   ```

   **方法六：配置 Docker 使用代理或更换镜像加速器**：
...
使用 rsync 上传代码（支持排除文件和增量同步）：

**步骤 1：了解 .rsyncignore 文件**

项目根目录已包含 `.rsyncignore` 文件，会自动排除以下内容：
- `node_modules/`（包括 frontend 和 backend 中的）
- `dist/`、`build/` 等构建输出
- `.git/`、日志、临时文件等
...
```bash
# 在本地电脑执行（替换为你的服务器 IP）
# 注意：路径末尾的 / 表示上传目录内容，不是整个目录
rsync -avz --exclude-from='.rsyncignore' \
  /path/to/knowlens/ root@<服务器IP>:/opt/knowlens/
```
...
- `-v`：显示详细输出
- `-z`：压缩传输
- `--exclude-from='.rsyncignore'`：使用项目根目录的 `.rsyncignore` 文件排除文件

**示例**（替换 `<服务器IP>` 为实际 IP）：
```bash
rsync -avz --exclude-from='.rsyncignore' \
  /Users/chenjialin/Desktop/WorkSpace/personal/knowlens/ root@60.205.120.189:/opt/knowlens/
```

## 🔐 第四步：配置环境变量(通过rsync传上去后不需要创建)

由于 `.env` 文件没有上传到 Git，需要在服务器上手动创建。
...
在执行构建之前，确保在正确的目录下，并且 `docker-compose.yml` 文件存在：
...
```bash
# 在本地电脑执行
rsync -avz docker-compose.yml root@60.205.120.189:/opt/knowlens/
```
...
由于服务器上的配置文件可能还是旧版本，需要重新上传：

**步骤 1：重新同步项目代码**

在本地电脑执行，重新同步整个项目（会排除 node_modules 等）：
...
4. **如果仍然失败，可以尝试直接使用阿里云容器镜像服务的镜像地址**：
   ```bash
   # 先手动拉取并重命名镜像
   docker pull registry.cn-hangzhou.aliyuncs.com/acs/node:20-alpine
...
**如果遇到 `npm error 'disturl' is not a valid npm option` 错误**：

这个错误是因为服务器上的 Dockerfile 文件还是旧版本，包含了已废弃的 `disturl` 配置选项。解决方法：

1. **重新上传 Dockerfile 文件**：
   ```bash
   # 在本地电脑执行
   rsync -avz backend/Dockerfile root@60.205.120.189:/opt/knowlens/backend/
   rsync -avz frontend/Dockerfile root@60.205.120.189:/opt/knowlens/frontend/
   ```
...
3. **验证 Dockerfile 内容**（确保没有 `disturl` 配置）：
   ```bash
   # 在服务器上检查
   grep -n "disturl" backend/Dockerfile frontend/Dockerfile
   # 如果没有任何输出，说明文件已正确
   ```

**如果遇到 `npm error code E401` 或 `Incorrect or missing password` 错误**：

这个错误是因为 npm 使用了缓存的认证信息，但认证已过期。解决方法：

1. **重新同步项目代码**（确保使用最新的 Dockerfile）：
   ```bash
   # 在本地电脑执行
   rsync -avz --exclude-from='.rsyncignore' \
     /Users/chenjialin/Desktop/WorkSpace/personal/knowlens/ root@60.205.120.189:/opt/knowlens/
...
最新的 Dockerfile 已经修复了这个问题，会在设置 registry 之前完全清理所有 npm 配置和认证信息。
