...
4. 配置参数：
   - **付费模式**：包年包月（推荐）或按量付费
   - **地域和可用区**：选择离用户最近的地域
   - **实例规格**：选择 **ecs.c6.large**（2 核 4G）或 **ecs.c6.xlarge**（4 核 8G）
   - **镜像**：选择 **Ubuntu 22.04 64 位**
   - **系统盘**：40GB 高效云盘
   - **网络**：选择专有网络 VPC
   - **公网 IP**：分配公网 IPv4 地址
...
| -------- | -------- | -------- | -------- | ----------------------- | ---------------------- | ------ |
| 入方向   | 允许     | TCP      | 22       | 你的 IP/32 或 0.0.0.0/0 | SSH 远程连接           | 1      |
...
1. **主域名**
   ...
   - **主机记录**：`@`
   - **记录类型**：A
   - **记录值**：你的服务器公网 IP（如 `60.205.120.189`）
   - **TTL**：600
...
ping mikestan.cn
ping www.mikestan.cn
ping knowlens.mikestan.cn
```

### 6.2 使用脚本申请 SSL 证书（推荐）

项目中已经包含了自动化的 SSL 证书申请脚本 `setup-ssl.sh`，支持同时为多个域名申请证书。
...
注意：要在域名能正常访问项目再执行以下步骤，DNS 未生效或者域名被运营商拦截都可能导致证书申请失败

```bash
# 格式：./setup-ssl.sh 主域名 邮箱 [额外域名...]
# 为 mikestan.cn、www.mikestan.cn 和 knowlens.mikestan.cn 申请证书

./setup-ssl.sh mikestan.cn crazyjialin@foxmail.com www.mikestan.cn knowlens.mikestan.cn
```

**参数说明：**

- 第 1 个参数：主域名（如 `mikestan.cn`）
- 第 2 个参数：邮箱地址（用于 Let's Encrypt 发送证书过期通知）
- 第 3 个及之后的参数：额外的域名（如 `www.mikestan.cn`、`knowlens.mikestan.cn`）

**注意：**

- 确保所有域名的 DNS 解析都已生效（可以先用 `ping` 命令测试）
- 如果只需要两个域名，可以省略第三个：`./setup-ssl.sh mikestan.cn crazyjialin@foxmail.com www.mikestan.cn`

##### 步骤 4：修改 nginx 配置

1. 在本地 VSCode 中打开 `frontend/nginx.conf`
2. 注释掉第 10-55 行（阶段 1 的 HTTP 配置）
   ```nginx
   # server {
   #     listen 80;
   #     server_name mikestan.cn www.mikestan.cn knowlens.mikestan.cn;
   #     ...
   # }
   ```
3. 取消注释第 67-135 行（阶段 2 的 HTTPS 配置）
   - 删除每行开头的 `#` 和一个空格
4. 保存后上传到服务器：`rsync -avz --exclude-from='.rsyncignore' . root@60.205.120.189:/opt/knowlens/`

##### 步骤 5：确认修改完成

回到脚本执行窗口，输入 `y` 确认修改完成。脚本会自动：
...
```bash
# 测试 HTTPS 访问
curl -I https://mikestan.cn
curl -I https://www.mikestan.cn
curl -I https://knowlens.mikestan.cn
```

或在浏览器中访问：`https://mikestan.cn`
...
2. 添加以下行（每月1号凌晨自动续期）：
   ```
   0 0 1 * * cd /root/knowlens && docker run --rm -v $(pwd)/certbot/conf:/etc/letsencrypt -v $(pwd)/certbot/www:/var/www/certbot certbot/certbot renew --quiet && docker compose restart frontend
   ```
3. 按 `Ctrl + O` 保存（显示 "File Name to Write"）
4. 按 `Enter` 确认
5. 按 `Ctrl + X` 退出
...
```bash
# 步骤1：先在服务器上停止容器（避免文件冲突）
ssh knowlens
cd /opt/knowlens
docker compose down
docker ps # 查看容器是否停止

# 步骤2：在本地机器执行 rsync 上传代码
rsync -avz --exclude-from='.rsyncignore' . root@60.205.120.189:/opt/knowlens/

# 步骤3：在服务器上重新构建并启动
ssh root@60.205.120.189
cd /opt/knowlens
...
# 步骤4：查看启动日志，确认服务正常
ssh root@60.205.120.189 "cd /opt/knowlens && docker compose logs -f"
```
...
# 2. 如果是旧的 nginx 或 Apache，停止它
sudo systemctl stop nginx
# 或
sudo systemctl stop apache2

# 3. 如果是其他 Docker 容器，停止它
docker ps | grep 80
docker stop <容器ID>
